...
public class CacheUtil {
    ...
    private Bitmap getBitmapFromDiskCache(String fileName){
        Bitmap bm = null;
        byte[] data = DisCacheUtil.getInstance(GlobalTool.getInstance().getContext()).readBytesFromStorage(fileName);
    ...
    class ChildCallback implements Handler.Callback{
        ...
        public boolean handleMessage(Message message) {
            ...
            String currentUsbPath = VideoManager.getInstance().getUsbRootPathByFilePath(task.path);
            Log.i(TAG, "===handleMessage=MSG_CALLBACK===isUsbMount=="+VideoManager.getInstance().isCurrentUsbMount(currentUsbPath)+" currentUsbPath： "+currentUsbPath);
            if (!VideoManager.getInstance().isCurrentUsbMount(currentUsbPath)){
                return false;
            }
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message message) {
        	...
        	case MSG_CALLBACK:
                ...
                String currentUsbPath = VideoManager.getInstance().getUsbRootPathByFilePath(task.path);
                Log.i(TAG, "===handleMessage=MSG_CALLBACK===isUsbMount=="+VideoManager.getInstance().isCurrentUsbMount(currentUsbPath)+" currentUsbPath： "+currentUsbPath);
        		if (task.callback != null && VideoManager.getInstance().isCurrentUsbMount(currentUsbPath)) {
	...
	public Task loadBitmapFromDB(Task task){
		...
		try {
			...
			DisCacheUtil.getInstance(GlobalTool.getInstance().getContext()).writeFileToStorage(task.path, Bitmap2Bytes(thumb));
		} catch (Exception e) {
			e.printStackTrace();
		}
		if (thumb == null) {
