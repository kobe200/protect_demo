package com.cookoo.musicsdkclient.utils;
...
public class LogUtils {
	 ...
	 * 存放日志文件的所在路径
	 */
	private static final String DEFAULT_DIR_PATH = "/HUA/log/1";
	/**
	 * 设置时间的格式
	 */
	private static final String DEFAULT_IN_FORMAT = "yyyy-MM-dd hh:mm:ss";
	/**
	 * 默认level
	 */
	private static int DEFAULT_LEVEL = Log.DEBUG;
	/**
	 * 存放日志的文本名
	 */
	private static final String DEFAULT_LOG_NAME = "S21A_iPod.txt";
	/**
	 * 默认显示log
	 */
	private static boolean IS_DEBUG = true;
	/**
	 * IS_WRITE:用于开关是否吧日志写入txt文件中
	 */
	...
	private static void performPrint(int level, String tag, String msg) {
		...
		Log.println(level, tag, "CarnetMedia_MusicExtend " + threadName + " " + lineIndicator
				+ " " + msg);
	...
	public static void write(String msg) {
		String path = createMkdirsAndFiles(DEFAULT_DIR_PATH, DEFAULT_LOG_NAME);
		if (TextUtils.isEmpty(path)) {
			return;
		}
		String threadName = Thread.currentThread().getName();
		String lineIndicator = getLineIndicator();
		String log = DateFormat.format(DEFAULT_IN_FORMAT,
