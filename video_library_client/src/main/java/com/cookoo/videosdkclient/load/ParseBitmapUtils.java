...
public class ParseBitmapUtils {
	...
	public static Task downloadBitmapByPath(Task task,int screenWidth,int screenHeight) {
		...
		Log.i(TAG," --->> downloadBitmapByPath() temp: "+temp);
		if (temp != null && temp.length > 0) {
			//将图片写到文件中
			DisCacheUtil.getInstance(VideoAidlManager.getInstance().getContext()).writeFileToStorage(task.path, temp);
			try {
			...
			task.parseState =  CacheUtil.ParseState.PICTURE_PARSE_FAILURE;
		}
		if (task.bitmap != null) {
			//将图片保存到LruCache中
			CacheUtil.getInstance().putBitmapIntoCache(task.path, task.bitmap);
			task.parseState =  CacheUtil.ParseState.PICTURE_PARSE_SUCCESS;
		}else{
			task.parseState =  CacheUtil.ParseState.PICTURE_PARSE_FAILURE;
			task.bitmap = null;
		}
	...
	public static byte[] decodeBitmap(String path,int width,int height) {
		Log.i(TAG," decodeBitmap() width: "+width+"  height: "+height+"  path: "+path);
		if (path == null) {
			return null;
		}
		...
		Log.i(TAG, "decodeBitmap==computeSampleSize=="+computeSampleSize(opts, -1, width * height));
