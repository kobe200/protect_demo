...
public class MediaItemInfo implements Parcelable {
	...
	public static enum MediaType {
		UNKNOWN (0), VIDEO (1), AUDIO (2), IMAGE (3), ALL (4);
	...
	public MediaItemInfo(MediaType mediaType, String name, String filePath, long size, long lastModified, String nameAlphabet) {
	...
	public void writeToParcel(Parcel dest,int flags){
		...
		dest.writeBooleanArray(new boolean[] { this.isDelete(), this.isPlayItem() });
	...
	public static final Creator<MediaItemInfo> CREATOR = new Creator<MediaItemInfo>() {
		...
		// 重写Creator
		@Override
		public MediaItemInfo createFromParcel(Parcel source) {
			...
			MediaItemInfo mediaItemInfo = new MediaItemInfo(mediaType, name, filePath,
					size, lastModified, nameAlphabet);
			mediaItemInfo.setTitle(title);
			mediaItemInfo.setAlbum(album);
			mediaItemInfo.setMediaID(mediaID);
			mediaItemInfo.setAlbumID(albumID);
			mediaItemInfo.setArtist(artist);
			mediaItemInfo.setScanIndex(scanIndex);
			mediaItemInfo.setPosition(position);
			mediaItemInfo.setUpdateTime(updateTime);
			mediaItemInfo.setParseStatus(parseStatus);
			mediaItemInfo.setDuration(duration);
			mediaItemInfo.setID(ID);
			mediaItemInfo.setDelete(booleanValues[0]);
			mediaItemInfo.setPlayItem(booleanValues[1]);
			mediaItemInfo.setPlaylistIndex(playlistIndex);
			mediaItemInfo.setNameAlphabet(nameAlphabet);
